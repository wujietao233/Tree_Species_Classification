...
class ClassTokenPosEmbed(nn.Module):
    ...
    def __init__(self, embed_dim):
        super(ClassTokenPosEmbed, self).__init__()
...
class Vision_Transformer(nn.Module):
    ...
    def __init__(self, num_classes=1000, depth=12, drop_ratio=0., embed_dim=768):
        ...
        # 实例化类别标签和位置编码
        self.cls_pos_embed = ClassTokenPosEmbed(embed_dim=embed_dim)

        # 位置编码后做dropout
        self.pos_drop = nn.Dropout(drop_ratio)

        # 在列表中添加12个EncoderBlock
        self.blocks = nn.Sequential(*[EncoderBlock(dim=embed_dim) for _ in range(depth)])
