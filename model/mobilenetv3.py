...
class MobileNetV3_large(nn.Module):
    ...
    def forward(self, x):
        # Hswish激活函数: 样本个数 批处理尺寸 宽 高 NBWH
        out = Hswish(self.bn1(self.conv1(x)))  # 2 16 112 112
        out = self.layers(out)  # 2 160 7 7
        out = Hswish(self.bn2(self.conv2(out)))  # 2 960 7 7
        out = F.avg_pool2d(out, 7)  # 2 960 1 1
        out = Hswish(self.conv3(out))  # 2 1280 1 1
        out = self.conv4(out)  # 2 17 1 1
        # 因为原论文中最后一层是卷积层来实现全连接的效果，维度是四维的，后两维是1，在计算损失函数的时候要求二维，因此在这里需要做一个resize
        a, b = out.size(0), out.size(1)
        out = out.view(a, b)  # 2 17
        return out
